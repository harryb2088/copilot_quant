...
def generate_cash_history(days=90):
    ...
    return pd.DataFrame({"Date": dates, "Cash": cash_history})
...
def generate_exposure_history(days=90):
    ...
    return pd.DataFrame({"Date": dates, "Gross_Exposure": gross_exposure, "Net_Exposure": net_exposure})
...
def generate_leverage_history(days=90):
    ...
    return pd.DataFrame({"Date": dates, "Leverage": leverage})
...
def generate_price_data_with_signals(symbol="AAPL", days=365):
    ...
    """
    if not USE_MOCK_DATA:
        return (pd.DataFrame({"Date": [], "Close": []}), pd.DataFrame({"timestamp": [], "type": [], "price": []}))
    ...
    for i, idx in enumerate(signal_indices):
        ...
        signals.append(
            {"timestamp": dates[idx], "type": signal_type, "price": prices[idx], "quantity": np.random.randint(10, 100)}
        )
