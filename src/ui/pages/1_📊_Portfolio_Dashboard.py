...
from components.charts import plot_dividend_history, plot_portfolio_performance_chart
...
# ============================================================================
st.set_page_config(
    page_title="Portfolio Dashboard | Bloomberg Style", page_icon="📊", layout="wide", initial_sidebar_state="expanded"
)
...
positions_data["Dividend Yield"] = positions_data["Symbol"].apply(lambda x: get_dividend_info(x)["dividend_yield"])
...
with col1:
    st.metric(label="Portfolio Value", value=f"${total_assets:,.0f}", delta=f"+${total_return:,.0f}")

with col2:
    st.metric(label="Total Return", value=f"{total_return_pct:.2f}%", delta="YTD")

with col3:
    st.metric(label="Cash", value=f"${cash_balance:,.0f}", delta=f"{(cash_balance / total_assets) * 100:.1f}%")

with col4:
    st.metric(label="Dividend Yield", value=f"{div_summary['portfolio_yield'] * 100:.2f}%", delta="Annual")
...
with col6:
    st.metric(label="Sharpe Ratio", value=f"{sharpe_ratio:.2f}", delta="1Y")
...
with div_col1:
    ...
    if not dividend_calendar.empty:
        ...
        st.dataframe(display_calendar, hide_index=True, use_container_width=True, height=250)
    else:
...
with div_col2:
    ...
    div_metrics = pd.DataFrame(
        {
            "Metric": ["Annual Income", "Quarterly Income", "Monthly Income", "Portfolio Yield", "Div Growth (Est.)"],
            "Value": [
...
st.dataframe(positions_display, hide_index=True, use_container_width=True, height=350)
