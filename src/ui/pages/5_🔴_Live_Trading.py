...
from components.ibkr_connection_panel import render_connection_info_expander, render_ibkr_connection_panel
...
from components.sidebar import render_connection_status, render_sidebar
...
# Page configuration
st.set_page_config(page_title="Live Trading - Copilot Quant", page_icon="🔴", layout="wide")
        ...
        **⚠️ Important Reminders:**
        - """
            + (
                ...
                else "⚠️ Live trading uses REAL MONEY"
            )
            + """
        - Execution may differ in live trading
        - Use this to """
            + ("learn and test strategies" if current_mode == "paper" else "execute real trades carefully")
            + """
        - """
...
# Footer
st.caption(
    f"🔒 Current Mode: {'Paper Trading (No real money)' if current_mode == 'paper' else '⚠️ LIVE TRADING (Real money at risk)'}"
)
