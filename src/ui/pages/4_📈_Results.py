...
from components.charts import plot_drawdown, plot_equity_curve, plot_monthly_returns_heatmap, plot_price_with_signals
...
from utils.mock_data import (
    ...
    generate_price_data_with_signals,
    generate_trades_pnl_table,
)
from utils.session import init_session_state

# Page configuration
st.set_page_config(page_title="Results - Copilot Quant", page_icon="📈", layout="wide")
...
if completed_backtests:
    backtest_options = [f"{bt['id']} - {bt['strategy']} ({bt['date_range']})" for bt in completed_backtests]

    selected_backtest = st.selectbox(
        "Select Backtest to View", backtest_options, help="Choose a completed backtest to view results"
    )
    ...
    with col1:
        st.metric(label="Total Return", value=metrics["total_return"], delta=metrics["total_return"])
    ...
    with col3:
        st.metric(
            label="Max Drawdown", value=metrics["max_drawdown"], delta=metrics["max_drawdown"], delta_color="inverse"
        )

    with col4:
        st.metric(label="Win Rate", value=metrics["win_rate"], delta="Profitable")

    with col5:
        st.metric(label="Total Trades", value=metrics["total_trades"], delta=f"{metrics['total_trades']} executed")
    ...
    with st.expander("📝 Add Notes"):
        notes = st.text_area(
            "Backtest Notes", placeholder="Add your observations, insights, or next steps here...", height=100
        )
