...
def plot_portfolio_performance_chart(dates, values, title="Portfolio Performance"):
    ...
    fig.update_layout(
        title=dict(text=f"<b>{title}</b>", font=dict(size=24, color=TEXT_COLOR), x=0.5, xanchor="center"),
        xaxis_title="",
        yaxis_title="Portfolio Value",
        plot_bgcolor=CARD_BG,
        paper_bgcolor=DARK_BG,
        font=dict(color=TEXT_COLOR, family="Arial, sans-serif", size=14),
        xaxis=dict(gridcolor=GRID_COLOR, showgrid=True, color=TEXT_COLOR),
        yaxis=dict(gridcolor=GRID_COLOR, showgrid=True, tickformat="$,.0f", color=TEXT_COLOR),
        hovermode="x unified",
        height=500,
...
def plot_returns_distribution(returns):
    ...
    fig.add_trace(go.Histogram(x=returns, nbinsx=50, name="Returns", marker_color="#2ca02c"))
...
def plot_monthly_returns_heatmap(data):
    ...
    fig = go.Figure(data=go.Heatmap(z=returns_data, x=months, y=years, colorscale="RdYlGn", zmid=0))

    fig.update_layout(
        title="Monthly Returns Heatmap", xaxis_title="Month", yaxis_title="Year", template="plotly_white", height=300
    )
...
def plot_exposure_chart(dates, gross_exposure, net_exposure):
    ...
    fig.add_trace(
        go.Scatter(x=dates, y=gross_exposure, mode="lines", name="Gross Exposure", line=dict(color="#1f77b4", width=2))
    )

    fig.add_trace(
        go.Scatter(x=dates, y=net_exposure, mode="lines", name="Net Exposure", line=dict(color="#ff7f0e", width=2))
    )
...
def plot_leverage_history(dates, leverage_values):
    ...
    fig.add_trace(
        go.Scatter(x=dates, y=leverage_values, mode="lines", name="Leverage Ratio", line=dict(color="#ff7f0e", width=2))
    )

    # Add reference line at 1.0
    fig.add_hline(y=1.0, line_dash="dash", line_color="gray", opacity=0.5, annotation_text="No Leverage")
...
def plot_allocation_pie(categories, values):
    ...
    fig.update_layout(title="Asset Allocation", height=350, showlegend=True)
...
def plot_sector_exposure(sectors, exposures):
    ...
    Returns:
        Plotly figure
    """
    fig = go.Figure(data=[go.Bar(x=exposures, y=sectors, orientation="h", marker=dict(color="#1f77b4"))])
...
def plot_attribution_waterfall(assets, pnl_values):
    ...
    for i, (asset, pnl) in enumerate(zip(assets, pnl_values, strict=False)):
        fig.add_trace(
            go.Bar(x=[asset], y=[pnl], base=[cumulative[i]], marker=dict(color=colors[i]), name=asset, showlegend=False)
        )
...
def plot_rolling_sharpe(dates, sharpe_values):
    ...
    fig.add_trace(
        go.Scatter(x=dates, y=sharpe_values, mode="lines", name="Rolling Sharpe", line=dict(color="#9467bd", width=2))
    )
...
def plot_price_with_signals(price_df, signals_df):
    ...
    fig.add_trace(
        go.Scatter(
            x=price_df["Date"], y=price_df["Close"], mode="lines", name="Price", line=dict(color="#1f77b4", width=2)
        )
    )
...
def plot_dividend_history(dividend_df, symbol="Portfolio"):
    ...
    fig.add_trace(
        go.Bar(
            ...
            name="Dividend",
            marker=dict(color=ACCENT_GREEN, line=dict(color=ACCENT_GREEN, width=1)),
            hovertemplate="<b>%{x|%Y-%m-%d}</b><br>Dividend: $%{y:.2f}<extra></extra>",
        )
    )

    fig.update_layout(
        title=dict(text=f"<b>{symbol} - Dividend History</b>", font=dict(size=16, color=TEXT_COLOR)),
