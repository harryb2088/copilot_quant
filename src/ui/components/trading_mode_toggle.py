...
def render_trading_mode_toggle(
    ...
    if session_key not in st.session_state:
        ...
        st.session_state[f"{session_key}_history"] = [
            {"timestamp": datetime.now().isoformat(), "mode": default_mode}
        ]
        ...
        if new_mode == "live":
            ...
            **Before proceeding:**
            ...
            confirm1 = st.checkbox(
                "I understand that live trading involves real money and real risk",
                key=f"{session_key}_confirm1"
            )
            
            confirm2 = st.checkbox(
                "I have tested my strategies and am ready to trade with real money",
                key=f"{session_key}_confirm2"
            )
            ...
            with col1:
                if st.button(
                    ...
                    # Log the mode switch
                    logger.warning(
                        f"⚠️ TRADING MODE SWITCHED: PAPER → LIVE at {datetime.now().isoformat()}"
                    )
            ...
            with col1:
                if st.button(
                    ...
                    # Log the mode switch
                    logger.info(
                        f"Trading mode switched: LIVE → PAPER at {datetime.now().isoformat()}"
                    )
            ...
            with col2:
                if st.button(
                    "Cancel",
                    use_container_width=True,
                    key=f"{session_key}_paper_cancel_btn"
                ):
