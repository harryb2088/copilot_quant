...
def render_ibkr_orders_panel(service, limit: int = 10) -> None:
    ...
    if 'Avg Fill Price' in display_df.columns:
        display_df['Avg Fill Price'] = display_df['Avg Fill Price'].apply(
    ...
    pending_orders = sum(1 for o in orders if str(o.get('status', '')).upper() in ['SUBMITTED', 'PENDINGSUBMIT', 'PRESUBMITTED'])
