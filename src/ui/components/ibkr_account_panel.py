...
def render_ibkr_account_panel(service) -> None:
    ...
    with col1:
        ...
        st.metric("Account Value", f"${net_liq:,.2f}", help="Total account value (cash + positions)")

    with col2:
        cash = account.get("total_cash_value", 0)
        st.metric("Cash", f"${cash:,.2f}", help="Available cash balance")

    with col3:
        buying_power = account.get("buying_power", 0)
        st.metric("Buying Power", f"${buying_power:,.2f}", help="Available buying power")
    ...
    with col5:
        ...
        st.metric("Realized P&L", f"${realized_pnl:,.2f}", help="Realized profit/loss from closed positions")
...
def render_account_metrics_compact(service) -> Optional[Dict[str, Any]]:
    ...
    st.metric("Account Value", f"${net_liq:,.0f}", delta=f"${unrealized_pnl:+,.0f}")
