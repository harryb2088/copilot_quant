...
def render_ibkr_positions_panel(service) -> None:
    ...
    with col1:
        st.metric("Total Positions", len(positions), help="Number of open positions")

    with col2:
        st.metric("Total Market Value", f"${total_market_value:,.2f}", help="Combined value of all positions")
