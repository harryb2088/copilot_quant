...
class RiskSettings:
    ...
    def validate(self) -> None:
        ...
        if not 0 < self.target_portfolio_volatility <= 1:
            raise ValueError(f"target_portfolio_volatility must be between 0 and 1, got {self.target_portfolio_volatility}")
        
        # Validate cash buffer logic
        if self.min_cash_buffer > self.max_cash_buffer:
            raise ValueError(f"min_cash_buffer ({self.min_cash_buffer}) cannot be greater than max_cash_buffer ({self.max_cash_buffer})")
