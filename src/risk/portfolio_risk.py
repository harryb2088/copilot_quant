...
class RiskManager:
    ...
    def check_portfolio_risk(
        ...
        if portfolio_value <= 0:
            return RiskCheckResult(
                approved=False, reason="Portfolio value must be positive", details={"portfolio_value": portfolio_value}
            )
        ...
        if self.settings.enable_circuit_breaker and current_drawdown >= self.settings.circuit_breaker_threshold:
            return self.trigger_circuit_breaker(
                portfolio_value=portfolio_value, peak_value=peak_value, current_drawdown=current_drawdown
            )
    ...
    def check_correlation(
        ...
        if not existing_symbols:
            return RiskCheckResult(
                approved=True, reason="No symbols in existing positions", details={"new_symbol": new_symbol}
            )

        # Check if we have price data for new symbol
        if new_symbol not in price_data.columns:
            return RiskCheckResult(
                approved=True, reason=f"No price data available for {new_symbol}", details={"new_symbol": new_symbol}
            )
