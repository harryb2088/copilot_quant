...
class TestFindCointegratedPairs:
    ...
    def test_find_pairs_with_filters(self):
        ...
        prices_df = pd.DataFrame({"A": [1, 2, 3, 4, 5], "B": [2, 4, 6, 8, 10], "C": [5, 4, 3, 2, 1]})

        pairs = find_cointegrated_pairs(prices_df, min_correlation=0.95)
    ...
    def test_insufficient_data(self):
        """Test with insufficient data."""
        prices_df = pd.DataFrame({"A": [1, 2, 3], "B": [2, 4, 6]})
