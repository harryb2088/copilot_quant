...
class MockPairsDataProvider(DataProvider):
    ...
    def get_historical_data(self, symbol: str, start_date=None, end_date=None, interval: str = "1d") -> pd.DataFrame:
    ...
    def get_multiple_symbols(
        self, symbols: List[str], start_date=None, end_date=None, interval: str = "1d"
    ) -> pd.DataFrame:
    ...
    def get_ticker_info(self, symbol: str) -> dict:
        ...
        return {"symbol": symbol, "name": f"{symbol} Test Company", "sector": "Technology", "industry": "Software"}
...
class TestPairsTradingStrategy:
    ...
    def test_strategy_initialization(self):
        ...
        strategy = PairsTradingStrategy(lookback=60, entry_zscore=2.0, exit_zscore=0.5, quantity=100, max_pairs=5)
    ...
    def test_strategy_identifies_pairs(self):
        ...
        engine = BacktestEngine(initial_capital=100000, data_provider=provider, commission=0.001, slippage=0.0005)

        strategy = PairsTradingStrategy(
            lookback=30, entry_zscore=2.0, exit_zscore=0.5, quantity=100, max_pairs=3, min_correlation=0.7
        )

        engine.add_strategy(strategy)

        result = engine.run(
            start_date=datetime(2023, 1, 1), end_date=datetime(2023, 6, 30), symbols=["PAIR_A", "PAIR_B", "INDEP"]
        )
    ...
    def test_strategy_entry_signals(self):
        ...
        result = engine.run(
            start_date=datetime(2023, 1, 1), end_date=datetime(2023, 12, 31), symbols=["PAIR_A", "PAIR_B"]
        )
    ...
    def test_strategy_with_insufficient_data(self):
        ...
        # Short date range
        result = engine.run(
            start_date=datetime(2023, 1, 1), end_date=datetime(2023, 1, 31), symbols=["PAIR_A", "PAIR_B"]
        )
    ...
    def test_strategy_performance_metrics(self):
        ...
        strategy = PairsTradingStrategy(lookback=30, entry_zscore=1.5, exit_zscore=0.5, quantity=100)
    ...
    def test_strategy_rebalance_frequency(self):
        ...
        result = engine.run(
            start_date=datetime(2023, 1, 1), end_date=datetime(2023, 3, 31), symbols=["PAIR_A", "PAIR_B"]
        )
...
class TestPairsTradingIntegration:
    ...
    def test_full_backtest_workflow(self):
        ...
        strategy = PairsTradingStrategy(lookback=30, entry_zscore=1.5, exit_zscore=0.5, quantity=100, max_pairs=2)
