...
class BuyAndHoldStrategy(Strategy):
    ...
    def on_data(self, timestamp, data):
        ...
        if not self.invested:
            self.invested = True
            return [Order(symbol=self.symbol, quantity=self.quantity, order_type="market", side="buy")]
        return []
...
class SimpleMomentumStrategy(Strategy):
    ...
    def on_data(self, timestamp, data):
        ...
        if current_price > ma and self.position <= 0:
            # Go long
            orders.append(Order(symbol=self.symbol, quantity=10, order_type="market", side="buy"))
            self.position = 1
        elif current_price < ma and self.position > 0:
            # Close long
            orders.append(Order(symbol=self.symbol, quantity=10, order_type="market", side="sell"))
            self.position = 0
...
class TestBacktestIntegration:
    ...
    def test_buy_and_hold_strategy_full_backtest(self):
        ...
        result = engine.run(start_date=datetime(2020, 1, 1), end_date=datetime(2020, 12, 31), symbols=["SPY"])
    ...
    def test_backtest_with_transaction_costs(self):
        ...
        # Run with no costs
        engine1 = BacktestEngine(initial_capital=100000, data_provider=provider, commission=0.0, slippage=0.0)
        strategy1 = BuyAndHoldStrategy(symbol="SPY", quantity=100)
        engine1.add_strategy(strategy1)
        result1 = engine1.run(start_date=datetime(2020, 1, 1), end_date=datetime(2020, 12, 31), symbols=["SPY"])
        ...
        result2 = engine2.run(start_date=datetime(2020, 1, 1), end_date=datetime(2020, 12, 31), symbols=["SPY"])
    ...
    def test_strategy_callbacks_called(self):
        ...
        engine.run(start_date=datetime(2020, 1, 1), end_date=datetime(2020, 12, 31), symbols=["SPY"])
