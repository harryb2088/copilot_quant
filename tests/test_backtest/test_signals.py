...
class TestTradingSignal:
    ...
    def test_invalid_side(self):
        ...
        with pytest.raises(ValueError, match="Invalid side"):
            TradingSignal(symbol="AAPL", side="invalid", confidence=0.8, sharpe_estimate=1.5, entry_price=150.0)

    def test_invalid_confidence_too_low(self):
        """Test that confidence below 0 raises ValueError."""
        with pytest.raises(ValueError, match="Invalid confidence"):
            TradingSignal(symbol="AAPL", side="buy", confidence=-0.1, sharpe_estimate=1.5, entry_price=150.0)

    def test_invalid_confidence_too_high(self):
        """Test that confidence above 1 raises ValueError."""
        with pytest.raises(ValueError, match="Invalid confidence"):
            TradingSignal(symbol="AAPL", side="buy", confidence=1.5, sharpe_estimate=1.5, entry_price=150.0)
    ...
    def test_invalid_entry_price(self):
        ...
        with pytest.raises(ValueError, match="Invalid entry_price"):
            TradingSignal(symbol="AAPL", side="buy", confidence=0.8, sharpe_estimate=1.5, entry_price=-10.0)

    def test_quality_score_calculation(self):
        """Test quality score calculation."""
        # Test with normal values
        signal1 = TradingSignal(symbol="AAPL", side="buy", confidence=0.8, sharpe_estimate=1.0, entry_price=150.0)
        ...
        signal2 = TradingSignal(symbol="AAPL", side="buy", confidence=0.8, sharpe_estimate=3.0, entry_price=150.0)
        ...
        signal3 = TradingSignal(symbol="AAPL", side="buy", confidence=1.0, sharpe_estimate=5.0, entry_price=150.0)
    ...
    def test_quality_score_with_low_confidence(self):
        ...
        signal = TradingSignal(symbol="AAPL", side="buy", confidence=0.2, sharpe_estimate=2.0, entry_price=150.0)
    ...
    def test_quality_score_edge_cases(self):
        ...
        # Zero confidence
        signal1 = TradingSignal(symbol="AAPL", side="buy", confidence=0.0, sharpe_estimate=2.0, entry_price=150.0)
        assert signal1.quality_score == pytest.approx(0.0)

        # Zero Sharpe
        signal2 = TradingSignal(symbol="AAPL", side="buy", confidence=1.0, sharpe_estimate=0.0, entry_price=150.0)
        assert signal2.quality_score == pytest.approx(0.0)

        # Both max
        signal3 = TradingSignal(symbol="AAPL", side="buy", confidence=1.0, sharpe_estimate=2.0, entry_price=150.0)
        assert signal3.quality_score == pytest.approx(1.0)
...
class TestSignalBasedStrategy:
    ...
    def test_simple_signal_generation(self):
        ...
        signals = strategy.generate_signals(datetime(2023, 1, 1), pd.DataFrame({"Close": [150.0]}))
        ...
        signals2 = strategy.generate_signals(datetime(2023, 1, 2), pd.DataFrame({"Close": [151.0]}))
    ...
    def test_on_data_returns_empty_list(self):
        ...
        orders = strategy.on_data(datetime(2023, 1, 1), pd.DataFrame({"Close": [150.0]}))
