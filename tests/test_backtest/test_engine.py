...
class TestBacktestEngine:
    ...
    def test_run_without_strategy_raises_error(self):
        ...
        with pytest.raises(ValueError, match="No strategy registered"):
            engine.run(start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 10), symbols=["AAPL"])
    ...
    def test_simple_backtest_runs(self):
        ...
        engine = BacktestEngine(initial_capital=10000, data_provider=provider, commission=0.001, slippage=0.0005)

        strategy = BuyOnceStrategy()
        engine.add_strategy(strategy)

        result = engine.run(start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 5), symbols=["AAPL"])
