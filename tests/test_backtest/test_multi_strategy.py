...
class TestMultiStrategyEngine:
    ...
    def test_engine_initialization(self):
        ...
        engine = MultiStrategyEngine(
            initial_capital=100000, data_provider=MockDataProvider(), max_position_pct=0.025, max_deployed_pct=0.80
        )
    ...
    def test_add_strategy(self):
        ...
        engine = MultiStrategyEngine(initial_capital=100000, data_provider=MockDataProvider())
    ...
    def test_run_without_strategies_raises_error(self):
        ...
        with pytest.raises(ValueError, match="No strategies registered"):
            engine.run(start_date=datetime(2023, 1, 1), end_date=datetime(2023, 1, 10), symbols=["AAPL"])

    def test_single_strategy_backtest(self):
        """Test backtest with a single strategy."""
        engine = MultiStrategyEngine(
            initial_capital=100000, data_provider=MockDataProvider(), commission=0.001, slippage=0.0005
        )

        strategy = BuyOnceStrategy(symbol="AAPL", confidence=0.8, sharpe=1.5)
        engine.add_strategy(strategy)

        result = engine.run(start_date=datetime(2023, 1, 1), end_date=datetime(2023, 1, 10), symbols=["AAPL"])
    ...
    def test_multiple_strategies_backtest(self):
        ...
        result = engine.run(start_date=datetime(2023, 1, 1), end_date=datetime(2023, 1, 10), symbols=["AAPL", "MSFT"])
...
class TestMaxDeploymentLimit:
    ...
    def test_max_deployed_prevents_excessive_trading(self):
        ...
        if not result.portfolio_history.empty and len(result.trades) > 0:
            max_deployed = (
                result.portfolio_history["positions_value"] / result.portfolio_history["portfolio_value"]
            ).max()

            # Allow small tolerance for rounding and slippage
            assert max_deployed <= engine.max_deployed_pct * 1.05, (
                f"Deployed capital ({max_deployed:.1%}) exceeded max limit ({engine.max_deployed_pct:.1%})"
            )
