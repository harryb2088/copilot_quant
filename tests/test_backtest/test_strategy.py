...
class TestStrategy:
    ...
    def test_strategy_on_fill_receives_fills(self):
        ...
        fill = Fill(order=order, fill_price=150.0, fill_quantity=100, commission=15.0, timestamp=datetime(2024, 1, 1))
...
class MovingAverageCrossStrategy(Strategy):
    ...
    def on_data(self, timestamp, data):
        ...
        if short_ma > long_ma and self.position <= 0:
            # Buy signal
            orders.append(Order(symbol=self.symbol, quantity=100, order_type="market", side="buy"))
            self.position = 1
        elif short_ma < long_ma and self.position >= 0:
            # Sell signal
            orders.append(Order(symbol=self.symbol, quantity=100, order_type="market", side="sell"))
            self.position = -1
