...
class TestMarketCalendar(unittest.TestCase):
    ...
    def test_market_state(self):
        ...
        self.assertEqual(self.calendar.get_market_state(pre_market), MarketState.PRE_MARKET)

        # Trading hours
        trading = datetime(2024, 3, 15, 10, 0, tzinfo=self.eastern)
        self.assertEqual(self.calendar.get_market_state(trading), MarketState.TRADING)

        # Post-market
        post_market = datetime(2024, 3, 15, 17, 0, tzinfo=self.eastern)
        self.assertEqual(self.calendar.get_market_state(post_market), MarketState.POST_MARKET)

        # Closed (weekend)
        closed = datetime(2024, 3, 16, 12, 0, tzinfo=self.eastern)
        self.assertEqual(self.calendar.get_market_state(closed), MarketState.CLOSED)
