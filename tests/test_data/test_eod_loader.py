...
def sample_symbols_file(temp_dir):
    ...
    df = pd.DataFrame(
        {"Symbol": ["AAPL", "MSFT", "GOOGL"], "Company": ["Apple Inc.", "Microsoft Corporation", "Alphabet Inc."]}
    )
...
class TestSP500EODLoader:
    ...
    def test_initialization_with_symbols(self, temp_dir):
        ...
        symbols = ["AAPL", "MSFT"]
        loader = SP500EODLoader(symbols=symbols, storage_type="csv", data_dir=temp_dir)
    ...
    def test_initialization_with_symbols_file(self, temp_dir, sample_symbols_file):
        ...
        loader = SP500EODLoader(symbols_file=sample_symbols_file, storage_type="csv", data_dir=temp_dir)
    ...
    def test_invalid_storage_type(self, temp_dir):
        ...
        with pytest.raises(ValueError, match="storage_type must be 'csv' or 'sqlite'"):
            SP500EODLoader(symbols=["AAPL"], storage_type="invalid", data_dir=temp_dir)

    def test_sqlite_database_initialization(self, temp_dir):
        """Test that SQLite database is properly initialized"""
        db_path = Path(temp_dir) / "test.db"
        SP500EODLoader(symbols=["AAPL"], storage_type="sqlite", db_path=str(db_path))
    ...
    def test_fetch_symbol_success(self, temp_dir):
        ...
        loader = SP500EODLoader(symbols=["AAPL"], storage_type="csv", data_dir=temp_dir)
    ...
    def test_fetch_invalid_symbol(self, temp_dir):
        ...
        loader = SP500EODLoader(symbols=["INVALID_SYMBOL_XYZ"], storage_type="csv", data_dir=temp_dir)
    ...
    def test_save_to_sqlite(self, temp_dir):
        ...
        db_path = Path(temp_dir) / "test.db"
        loader = SP500EODLoader(symbols=["AAPL"], storage_type="sqlite", db_path=str(db_path))
    ...
    def test_continue_on_error(self, temp_dir):
        ...
        loader = SP500EODLoader(symbols=["AAPL", "INVALID_XYZ", "MSFT"], storage_type="csv", data_dir=temp_dir)

        end_date = datetime.now().strftime("%Y-%m-%d")
        start_date = (datetime.now() - timedelta(days=5)).strftime("%Y-%m-%d")

        result = loader.fetch_all(start_date=start_date, end_date=end_date, continue_on_error=True)
    ...
    def test_split_handling_in_data(self, temp_dir):
        ...
        """
        loader = SP500EODLoader(symbols=["TSLA"], storage_type="sqlite", db_path=str(Path(temp_dir) / "test_splits.db"))
    ...
    def test_missing_symbol_error_handling(self, temp_dir):
        ...
        """
        loader = SP500EODLoader(symbols=["NOTREAL123", "FAKE456", "BOGUS789"], storage_type="csv", data_dir=temp_dir)
    ...
    def test_dividend_data_capture(self, temp_dir):
        ...
        """
        loader = SP500EODLoader(symbols=["DIV"], storage_type="csv", data_dir=temp_dir)
