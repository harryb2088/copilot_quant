...
class TestContractRollAdjustment:
    ...
    def test_adjust_for_contract_roll_manual_difference(self):
        ...
        result = adjust_for_contract_roll(
            df, roll_date="2024-01-03", adjustment=-0.25, front_contract_column="close", method="difference"
        )
    ...
    def test_adjust_for_contract_roll_manual_ratio(self):
        ...
        result = adjust_for_contract_roll(
            df, roll_date="2024-01-03", adjustment=1.1, front_contract_column="close", method="ratio"
        )
...
class TestNormalizationIdempotence:
    ...
    def test_timestamp_normalization_idempotent(self):
        ...
        df = pd.DataFrame({"date": pd.date_range("2024-01-01", periods=3, tz="UTC"), "price": [100, 101, 102]})
...
class TestNormalizationEdgeCases:
    ...
    def test_timezone_edge_cases(self):
        ...
        df = pd.DataFrame(
            {"date": pd.date_range("2024-03-10", periods=3, freq="h", tz="America/New_York"), "price": [100, 101, 102]}
        )
        ...
        df_naive = pd.DataFrame({"date": pd.date_range("2024-01-01", periods=3, freq="h"), "price": [100, 101, 102]})
    ...
    def test_duplicate_timestamps(self):
        ...
        df = pd.DataFrame(
            {"close": [100, 100.5, 101, 101.5], "volume": [1000, 2000, 3000, 4000]},
