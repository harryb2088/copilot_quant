...
from copilot_quant.brokers.order_execution_handler import Fill, OrderRecord
from copilot_quant.brokers.trade_reconciliation import Discrepancy, DiscrepancyType, ReconciliationReport
...
class TestAuditTrail(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.audit = AuditTrail(self.mock_broker, database_url="sqlite:///:memory:")
    ...
    def test_reconcile_today(self):
        ...
        reports = self.audit.database.get_reconciliation_reports(start_date=date.today(), end_date=date.today())
    ...
    def test_generate_compliance_report_json(self):
        ...
        report = self.audit.generate_compliance_report(date.today(), date.today(), output_format="json")
    ...
    def test_generate_compliance_report_text(self):
        ...
        report = self.audit.generate_compliance_report(date.today(), date.today(), output_format="text")
    ...
    def test_generate_compliance_report_invalid_format(self):
        ...
        with self.assertRaises(ValueError):
            self.audit.generate_compliance_report(date.today(), date.today(), output_format="xml")
    ...
    def test_export_audit_trail_json(self):
        ...
        try:
            self.audit.export_audit_trail(date.today(), date.today(), output_file=temp_file)
    ...
    def test_get_reconciliation_summary(self):
        ...
        summary = self.audit.get_reconciliation_summary(start_date=date.today(), end_date=date.today())
    ...
    def test_check_compliance_pass(self):
        ...
        status = self.audit.check_compliance(date.today(), date.today(), max_discrepancies=0)
    ...
    def test_check_compliance_fail(self):
        ...
        report.discrepancies.append(
            Discrepancy(type=DiscrepancyType.MISSING_LOCAL, order_id=1, symbol="AAPL", description="Test discrepancy")
        )
        self.audit.database.store_reconciliation_report(report)
