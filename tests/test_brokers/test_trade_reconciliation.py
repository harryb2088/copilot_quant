...
class TestReconciliationReport(unittest.TestCase):
    ...
    def test_report_has_discrepancies(self):
        ...
        report.discrepancies.append(
            trade_reconciliation.Discrepancy(
                type=trade_reconciliation.DiscrepancyType.MISSING_LOCAL, order_id=1, symbol="AAPL"
            )
        )
    ...
    def test_report_summary(self):
        ...
        report = trade_reconciliation.ReconciliationReport(
            ...
            ibkr_fills=[trade_reconciliation.IBKRFill("exec1", 1, "AAPL", "BUY", 100, 150.0, 1.0, datetime.now())],
            local_fills=[trade_reconciliation.LocalFill("fill1", 1, "AAPL", "BUY", 100, 150.0, 1.0, datetime.now())],
        )
...
class TestTradeReconciliation(unittest.TestCase):
    ...
    def test_fetch_local_fills_success(self):
        ...
        mock_order = OrderRecord(
            order_id=1, symbol="AAPL", action="BUY", total_quantity=100, order_type="MARKET", status=OrderStatus.FILLED
        )
    ...
    def test_reconcile_price_mismatch(self):
        ...
        price_mismatches = [
            d for d in report.discrepancies if d.type == trade_reconciliation.DiscrepancyType.PRICE_MISMATCH
        ]
        self.assertGreater(len(price_mismatches), 0)

    def test_reconcile_today(self):
        """Test reconciling today's trades"""
        self.reconciler.reconcile = MagicMock(
            return_value=trade_reconciliation.ReconciliationReport(reconciliation_date=date.today())
        )
