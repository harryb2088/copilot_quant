...
class TestLiveSignalMonitor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("copilot_quant.live.live_signal_monitor.TradeDatabase"):
            with patch(
                "copilot_quant.live.live_signal_monitor.LiveDataFeedAdapter", return_value=MockDataFeedAdapter()
            ):
                with patch(
                    "copilot_quant.live.live_signal_monitor.LiveBrokerAdapter", return_value=MockBrokerAdapter()
                ):
                    self.monitor = LiveSignalMonitor(
                        database_url="sqlite:///:memory:", paper_trading=True, update_interval=1.0
                    )
    ...
    def test_generate_all_signals(self):
        ...
        signal1 = TradingSignal(
            symbol="AAPL", side="buy", confidence=0.8, sharpe_estimate=1.5, entry_price=150.0, strategy_name="Strategy1"
        )

        signal2 = TradingSignal(
            symbol="MSFT", side="buy", confidence=0.7, sharpe_estimate=1.2, entry_price=250.0, strategy_name="Strategy2"
        )
    ...
    def test_risk_check_pass(self):
        ...
        signal = TradingSignal(symbol="AAPL", side="buy", confidence=0.8, sharpe_estimate=1.5, entry_price=150.0)
        ...
        with patch("copilot_quant.live.live_signal_monitor.TradeDatabase"):
            with patch(
                ...
                with patch(
                    ...
                    self.monitor = LiveSignalMonitor(database_url="sqlite:///:memory:", paper_trading=True)
                    self.monitor.data_feed = MockDataFeedAdapter()
                    self.monitor.broker = MockBrokerAdapter()
