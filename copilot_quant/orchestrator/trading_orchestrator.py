...
class TradingOrchestrator:
    ...
    def __init__(
        ...
        self.market_calendar = MarketCalendar(timezone=self.config.schedule.timezone)
    ...
    def _setup_notifiers(self) -> None:
        ...
        if "discord" in self.config.notifications.channels:
            if self.config.notifications.discord_webhook_url:
                self._notifiers.append(
                    DiscordNotifier(webhook_url=self.config.notifications.discord_webhook_url, min_level=min_level)
                )
        ...
        if "email" in self.config.notifications.channels:
            if (
                self.config.notifications.smtp_host
                and self.config.notifications.smtp_username
                and self.config.notifications.email_to
            ):
                self._notifiers.append(
                    EmailNotifier(
    ...
    def stop(self) -> None:
        ...
        self._notify(
            NotificationMessage(
                title="Orchestrator Stopped", message="Trading orchestrator daemon has stopped", level=AlertLevel.INFO
            )
        )
    ...
    def _stop_trading_engine(self) -> None:
        ...
        with self._engine_lock:
            ...
            try:
                ...
                self._notify(
                    NotificationMessage(
                        title="Trading Stopped", message="Trading engine stopped successfully", level=AlertLevel.INFO
                    )
                )
            ...
            if self.state == OrchestratorState.TRADING and self._start_time:
                ...
                self._notify(
                    NotificationMessage(
                        ...
                        message="Trading orchestrator is active and healthy",
                        level=AlertLevel.INFO,
                        metadata={"state": self.state.value, "uptime": str(timedelta(seconds=int(uptime_seconds)))},
                    )
                )
