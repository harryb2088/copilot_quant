...
class OrderNotificationAdapter:
    ...
    def __init__(
        ...
        logger.info(
            f"OrderNotificationAdapter initialized with {len(notifiers)} notifier(s)"
        )
...
class RiskNotificationAdapter:
    ...
    def __init__(self, notifiers: List[Notifier]):
        ...
        self.notifiers = notifiers
        logger.info(
            f"RiskNotificationAdapter initialized with {len(notifiers)} notifier(s)"
        )
    ...
    def on_portfolio_drawdown_warning(
        self,
        ...
        """
        message = (
            f"Portfolio drawdown at {current_drawdown*100:.2f}%, "
            f"approaching threshold of {threshold*100:.2f}%"
        )
