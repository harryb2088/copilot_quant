...
class LiveSignalMonitor:
    ...
    def __init__(
        ...
        # Statistics
        self.stats = {"total_signals_generated": 0, "signals_executed": 0, "signals_rejected": 0, "errors": 0}
    ...
    def add_strategy(self, strategy: SignalBasedStrategy) -> None:
        ...
        if not isinstance(strategy, SignalBasedStrategy):
            raise ValueError(f"Strategy must be instance of SignalBasedStrategy, got {type(strategy).__name__}")
    ...
    def start(self, symbols: List[str], lookback_days: int = 30, data_interval: str = "1d") -> bool:
        """
    ...
    def _generate_all_signals(self, timestamp: datetime, data: pd.DataFrame) -> List[TradingSignal]:
        """
        ...
        for strategy in self.strategies:
            try:
                ...
                logger.error(f"Error generating signals from {strategy.name}: {e}", exc_info=True)
    ...
    def _process_signal(self, signal: TradingSignal, timestamp: datetime) -> None:
        ...
        if self.enable_risk_checks:
            if not self._risk_check(signal):
                logger.info(f"Signal rejected by risk check: {signal.symbol} {signal.side}")
        ...
        if quantity <= 0:
            logger.info(f"Position size too small for signal: {signal.symbol} {signal.side}")
            self.stats["signals_rejected"] += 1
            return

        # Create order
        order = Order(symbol=signal.symbol, side=signal.side, quantity=quantity, order_type="market")
    ...
    def _risk_check(self, signal: TradingSignal) -> bool:
        ...
        if exposure_ratio >= self.max_total_exposure:
            logger.warning(
                f"Signal rejected - total exposure {exposure_ratio:.1%} >= max {self.max_total_exposure:.1%}"
            )
            return False

        # Check signal quality
        if signal.quality_score < 0.3:
            logger.info(f"Signal rejected - quality score {signal.quality_score:.2f} too low")
            return False
    ...
    def _load_historical_data(self, symbols: List[str], lookback_days: int, interval: str) -> None:
        """
