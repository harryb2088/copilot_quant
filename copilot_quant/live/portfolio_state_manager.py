...
class PortfolioStateManager:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if last_snapshot:
                ...
                logger.info(
                    f"Loaded last snapshot: NAV=${last_snapshot.nav:,.2f} "
                    f"from {last_snapshot.timestamp}"
                )
            else:
    ...
    def sync_with_broker(self) -> bool:
        ...
        try:
            ...
            if nav_diff_pct > 1.0:
                logger.warning(
                    f"NAV discrepancy: IBKR=${ibkr_nav:,.2f} vs "
                    f"Local=${local_nav:,.2f} (diff={nav_diff_pct:.2f}%)"
                )
            else:
                logger.info(
                    f"✓ Synced with broker: NAV=${ibkr_nav:,.2f}, "
                    f"Positions={len(ibkr_positions)}"
                )
    ...
    def take_snapshot(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✓ Snapshot taken: NAV=${nav:,.2f}, Drawdown={drawdown:.2%}, "
    ...
    def get_equity_curve(self, days: int = 30) -> pd.DataFrame:
        ...
        with self._get_session() as session:
            snapshots = session.query(PortfolioSnapshotModel).filter(
                PortfolioSnapshotModel.snapshot_date >= start_date
            ).order_by(
                PortfolioSnapshotModel.timestamp
            ).all()
    ...
    def get_reconciliation_history(
        ...
        with self._get_session() as session:
            logs = session.query(ReconciliationLogModel).filter(
                ReconciliationLogModel.reconciliation_date >= start_date
            ).order_by(
                ReconciliationLogModel.timestamp.desc()
            ).all()
    ...
    def _get_latest_snapshot(self) -> Optional[PortfolioSnapshotModel]:
        ...
        try:
            snapshot = session.query(PortfolioSnapshotModel).order_by(
