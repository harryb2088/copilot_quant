    ...
    def __post_init__(self):
        ...
        if not self.use_gateway:
            ...
            if self.mode == TradingMode.PAPER and self.port not in [7497]:
                logger.warning(f"Paper trading mode typically uses port 7497 (TWS), but configured port is {self.port}")
            elif self.mode == TradingMode.LIVE and self.port not in [7496]:
                logger.warning(f"Live trading mode typically uses port 7496 (TWS), but configured port is {self.port}")
        else:
            # Gateway ports
            if self.mode == TradingMode.PAPER and self.port not in [4002]:
                logger.warning(
                    f"Paper trading mode typically uses port 4002 (Gateway), but configured port is {self.port}"
                )
            elif self.mode == TradingMode.LIVE and self.port not in [4001]:
                logger.warning(
                    f"Live trading mode typically uses port 4001 (Gateway), but configured port is {self.port}"
                )
...
class TradingModeManager:
    ...
    def __init__(self, default_mode: TradingMode = TradingMode.PAPER):
        ...
        self._mode_history: list[tuple[datetime, TradingMode]] = [(datetime.now(), default_mode)]
    ...
    def switch_mode(self, new_mode: TradingMode, confirmed: bool = False) -> bool:
        ...
        logger.warning(
            f"⚠️  TRADING MODE SWITCHED: {old_mode.value.upper()} → {new_mode.value.upper()} at {timestamp.isoformat()}"
        )
