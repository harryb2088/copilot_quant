...
class PairsTradingStrategy(Strategy):
    ...
    def _identify_trading_pairs(self, data: pd.DataFrame) -> None:
        ...
        if "Symbol" in data.columns:
            ...
            prices_df = data.pivot_table(index=data.index, columns="Symbol", values="Close")
        else:
        ...
        pairs = find_cointegrated_pairs(
            recent_prices, significance_level=self.cointegration_pvalue, min_correlation=self.min_correlation
        )

        # Select top pairs (up to max_pairs)
        self.trading_pairs = [(sym1, sym2) for sym1, sym2, pval, corr in pairs[: self.max_pairs]]
    ...
    def _evaluate_pair(self, pair: Tuple[str, str], timestamp: datetime, data: pd.DataFrame) -> List[Order]:
        """
        ...
        if current_position == 0:
            if current_zscore > self.entry_zscore:
                ...
                orders.append(Order(symbol=sym1, quantity=self.quantity, order_type="market", side="sell"))
                orders.append(
                    Order(symbol=sym2, quantity=int(self.quantity * hedge_ratio), order_type="market", side="buy")
                )
                ...
                orders.append(Order(symbol=sym1, quantity=self.quantity, order_type="market", side="buy"))
                orders.append(
                    Order(symbol=sym2, quantity=int(self.quantity * hedge_ratio), order_type="market", side="sell")
                )
                self.pair_positions[pair] = 1
