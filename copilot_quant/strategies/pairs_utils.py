...
def calculate_correlation(series1: pd.Series, series2: pd.Series, method: str = "pearson") -> float:
    """
...
def calculate_hedge_ratio(series1: pd.Series, series2: pd.Series, method: str = "ols") -> float:
    """
    ...
    if method == "ols":
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(df["s2"], df["s1"])
        return slope
    else:
...
def calculate_spread(series1: pd.Series, series2: pd.Series, hedge_ratio: float = None) -> pd.Series:
    """
...
def calculate_zscore(series: pd.Series, window: int = None, min_periods: int = None) -> pd.Series:
    """
...
def find_cointegrated_pairs(
    prices: pd.DataFrame, significance_level: float = 0.05, min_correlation: float = 0.5
