...
class IPortfolioManager(ABC):
    ...
    @abstractmethod
    def update_position(self, symbol: str, fill: Fill, current_price: float) -> None:
        """
...
class IPerformanceAnalyzer(ABC):
    ...
    @abstractmethod
    def generate_report(self, equity_curve: pd.Series, trades: List[Fill], initial_capital: float) -> Dict:
        """
...
class IResultsTracker(ABC):
    ...
    @abstractmethod
    def record_portfolio_state(self, timestamp: datetime, portfolio_snapshot: Dict) -> None:
        """
...
class IRiskManager(ABC):
    ...
    def calculate_position_size(
        self, symbol: str, signal_strength: float, portfolio_value: float, risk_params: Dict
    ) -> float:
        """
