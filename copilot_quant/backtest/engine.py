...
class BacktestEngine:
    ...
    def __init__(
        self, initial_capital: float, data_provider: DataProvider, commission: float = 0.001, slippage: float = 0.0005
    ):
        """
    ...
    def run(self, start_date: datetime, end_date: datetime, symbols: List[str]) -> BacktestResult:
        """
        ...
        logger.info(f"Starting backtest: {start_date.date()} to {end_date.date()}, symbols={symbols}")
        ...
        logger.info(
            f"Backtest complete: Final value=${final_value:,.2f}, Return={total_return:.2%}, Trades={len(self.fills)}"
        )
    ...
    def _fetch_data(self, symbols: List[str], start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """Fetch historical data for symbols."""
        try:
            if len(symbols) == 1:
                # Single symbol - simpler data structure
                data = self.data_provider.get_historical_data(symbols[0], start_date=start_date, end_date=end_date)
                # Add symbol column for consistency
                if not data.empty:
                    data["Symbol"] = symbols[0]
            else:
                # Multiple symbols
                data = self.data_provider.get_multiple_symbols(symbols, start_date=start_date, end_date=end_date)
    ...
    def _get_current_data(self, data: pd.DataFrame, timestamp: datetime, symbols: List[str]) -> pd.DataFrame:
        """
    ...
    def _execute_order(self, order: Order, timestamp: datetime, current_data: pd.DataFrame) -> None:
        """
        ...
        if order.side == "buy":
            ...
            if total_cost > self.cash:
                logger.warning(f"Insufficient capital for order: need ${total_cost:,.2f}, have ${self.cash:,.2f}")
                return
        ...
        logger.debug(f"Executed: {order.side} {order.quantity} {order.symbol} @ ${fill_price:.2f}")
