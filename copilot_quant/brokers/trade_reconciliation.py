...
class ReconciliationReport:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "discrepancy_types": {
                dtype.value: sum(1 for d in self.discrepancies if d.type == dtype) for dtype in DiscrepancyType
            },
        }
...
class TradeReconciliation:
    ...
    def __init__(self, broker, price_tolerance: float = 0.01, commission_tolerance: float = 0.01):
        """
        ...
        logger.info(
            f"TradeReconciliation initialized with price_tolerance=${price_tolerance}, "
    ...
    def fetch_ibkr_fills(self, start_date: Optional[date] = None) -> List[IBKRFill]:
        ...
        try:
            ...
            if start_date:
                fills = [f for f in fills if f.time.date() >= start_date]
    ...
    def reconcile(self, target_date: date) -> ReconciliationReport:
        ...
        # Initialize report
        report = ReconciliationReport(reconciliation_date=target_date, ibkr_fills=ibkr_fills, local_fills=local_fills)
        ...
        for order_id in all_order_ids:
            ...
            if not ibkr_order_fills and local_order_fills:
                ...
                ibkr_avg_price = (
                    sum(f.quantity * f.price for f in ibkr_order_fills) / ibkr_total_qty if ibkr_total_qty > 0 else 0
                )
                local_avg_price = (
                    sum(f.quantity * f.price for f in local_order_fills) / local_total_qty if local_total_qty > 0 else 0
                )
        ...
        logger.info(
            f"Reconciliation complete: {len(report.matched_order_ids)} matched, "
