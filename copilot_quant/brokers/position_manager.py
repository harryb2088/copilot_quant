    ...
    def _detect_and_log_changes(
        ...
        for symbol, pos in current.items():
            if symbol not in previous:
                ...
                logger.info(
                    f"Position opened - {symbol}: {pos.quantity} shares @ ${pos.avg_cost:.2f}"
                )
        ...
        for symbol, pos in previous.items():
            if symbol not in current:
                ...
                logger.info(
                    f"Position closed - {symbol}: {pos.quantity} shares, "
        ...
        for symbol in set(previous.keys()) & set(current.keys()):
            ...
            if abs(curr_pos.quantity - prev_pos.quantity) > 0.0001:
                ...
                logger.info(
                    f"Position {change_type} - {symbol}: "
                    f"{prev_pos.quantity:.2f} -> {curr_pos.quantity:.2f} shares"
                )
            ...
            if previous_pos:
                if abs(position.quantity - previous_pos.quantity) > 0.0001:
                    logger.info(
                        ...
                        f"{previous_pos.quantity:.2f} -> {position.quantity:.2f} shares"
                    )
            else:
                logger.info(
                    f"Position opened via update - {symbol}: "
                    f"{position.quantity} shares @ ${position.avg_cost:.2f}"
                )
