...
class LiveBrokerAdapter(IBroker):
    ...
    def execute_order(self, order: Order, current_price: float, timestamp: datetime) -> Optional[Fill]:
        """
        ...
        try:
            ...
            if not self.check_buying_power(order, current_price):
                logger.warning(f"Insufficient buying power for order: {order.side} {order.quantity} {order.symbol}")
                return None

            # Execute order through IBKR
            if order.order_type.lower() == "market":
                order_record = self._broker.execute_market_order(
                    symbol=order.symbol, quantity=order.quantity, side=order.side
                )
                ...
                order_record = self._broker.execute_limit_order(
                    symbol=order.symbol, quantity=order.quantity, limit_price=order.limit_price, side=order.side
                )
            else:
    ...
    def check_buying_power(self, order: Order, price: float) -> bool:
        ...
        try:
            ...
            if not has_power:
                logger.warning(f"Insufficient buying power: need ${total_cost:,.2f}, have ${buying_power:,.2f}")
