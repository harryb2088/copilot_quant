...
class AuditTrail:
    ...
    def __init__(self, broker, database_url: str = "sqlite:///trade_audit.db",
    ...
    def _format_text_report(self, audit_data: Dict[str, Any]) -> str:
        ...
        if audit_data['orders']:
            ...
            for order in audit_data['orders']:
                lines.append(f"Order {order['order_id']}: {order['action']} {order['total_quantity']} "
        ...
        if audit_data['fills']:
            ...
            for fill in audit_data['fills']:
                lines.append(f"Fill {fill['fill_id']}: Order {fill['order_id']} - "
    ...
    def get_reconciliation_summary(self, start_date: Optional[date] = None,
    ...
    def check_compliance(self, start_date: date, end_date: date,
                        max_discrepancies: int = 0) -> Dict[str, Any]:
        """
