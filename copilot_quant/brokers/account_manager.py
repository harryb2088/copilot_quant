...
class IBKRAccountManager:
    ...
    def sync_account_state(self) -> bool:
        ...
        try:
            ...
            self._account_values_cache = {f"{av.tag}_{av.currency}": av for av in account_values}
    ...
    def get_all_account_values(self) -> Dict[str, Any]:
        ...
        """
        result = {}
        for _key, av in self._account_values_cache.items():
            try:
        ...
        if old_value:
            try:
                ...
                if abs(new_float - old_float) > 0.01:
                    logger.debug(f"Account value updated - {av.tag}: {old_float:.2f} -> {new_float:.2f}")
            except (ValueError, TypeError):
                pass
