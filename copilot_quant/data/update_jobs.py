...
class DataUpdater:
    ...
    def __init__(
        ...
        self.loader = SP500EODLoader(
            storage_type=storage_type, data_dir=str(data_dir), db_path=str(db_path), rate_limit_delay=rate_limit_delay
        )
    ...
    def update_symbol(self, symbol: str, force: bool = False, max_age_days: int = 1) -> bool:
        """
        ...
        try:
            ...
            # Fetch new data
            success = self.loader.fetch_and_save(symbol, start_date=start_date, end_date=end_date)
    ...
    def backfill_symbol(self, symbol: str, start_date: str, end_date: Optional[str] = None) -> bool:
        """
    ...
    def batch_update(
        self, symbols: List[str], force: bool = False, max_age_days: int = 1, continue_on_error: bool = True
    ) -> Dict[str, List[str]]:
        """
    ...
    def batch_backfill(
        self, symbols: List[str], start_date: str, end_date: Optional[str] = None, continue_on_error: bool = True
    ) -> Dict[str, List[str]]:
        """
    ...
    def _update_metadata(self, symbol: str):
        ...
        if mask.any():
            ...
            new_entry = pd.DataFrame({"symbol": [symbol], "last_update": [now], "last_date": [last_date]})
        ...
        try:
            ...
            for i in range(1, len(dates)):
                ...
                if days_diff > 3:  # Account for weekends
                    gaps.append((dates.iloc[i - 1].strftime("%Y-%m-%d"), dates.iloc[i].strftime("%Y-%m-%d")))
    ...
    def fill_gaps(self, symbol: str) -> bool:
        ...
        for start_date, end_date in gaps:
            try:
                ...
                self.loader.fetch_and_save(symbol, start_date=start_date, end_date=end_date)
