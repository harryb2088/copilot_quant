...
def normalize_timestamps(
    df: pd.DataFrame, market_type: str = "equity", target_timezone: Optional[str] = None, timestamp_column: str = "date"
) -> pd.DataFrame:
    """
...
def adjust_for_contract_roll(
    ...
    if adjustment is None:
        if back_contract_column is None:
            raise ValueError("Either 'adjustment' or 'back_contract_column' must be provided")
...
def calculate_adjusted_close(df: pd.DataFrame, use_dividends: bool = True, use_splits: bool = True) -> pd.DataFrame:
    """
...
def detect_missing_data(df: pd.DataFrame) -> Dict[str, List]:
    ...
    """
    issues = {"missing_values": [], "date_gaps": [], "zero_volume": [], "price_anomalies": []}

    # Check for missing values
    for col in df.columns:
        null_indices = df[df[col].isnull()].index.tolist()
        if null_indices:
            issues["missing_values"].extend([{"column": col, "index": idx} for idx in null_indices])
    ...
    if "date" in df.columns or isinstance(df.index, pd.DatetimeIndex):
        ...
        for i in range(1, len(dates)):
            ...
            if days_diff > 3:
                issues["date_gaps"].append({"start": dates.iloc[i - 1], "end": dates.iloc[i], "days": days_diff})
...
def fill_missing_data(df: pd.DataFrame, method: str = "ffill", limit: Optional[int] = None) -> pd.DataFrame:
    """
...
def remove_outliers(
    df: pd.DataFrame, column: str = "close", method: str = "iqr", threshold: float = 3.0
) -> pd.DataFrame:
    """
...
def resample_data(df: pd.DataFrame, freq: str = "D", aggregation: Optional[Dict[str, str]] = None) -> pd.DataFrame:
    """
