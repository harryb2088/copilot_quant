...
class PerformanceEngine:
    ...
    def __init__(self, trade_db: TradeDatabase, initial_capital: float, risk_free_rate: float = 0.02):
        """
    ...
    def get_current_performance(
        self, current_positions: Dict[str, Dict[str, float]], current_cash: float
    ) -> PerformanceSnapshot:
        """
        ...
        for _symbol, pos in current_positions.items():
    ...
    def get_historical_snapshots(
        self, start_date: date, end_date: date, frequency: str = "daily"
    ) -> List[Dict[str, Any]]:
        """
    ...
    def export_performance_report(self, start_date: date, end_date: date, format: str = "json") -> str:
        """
    ...
    def _calculate_realized_pnl(self) -> float:
        ...
        with self.trade_db.get_session() as session:
            ...
            for fill in fills:
                ...
                sum(q for q, p in positions[symbol])
    ...
    def _build_equity_curve(self, current_positions: Dict[str, Dict[str, float]], current_cash: float) -> pd.Series:
        ...
        with self.trade_db.get_session() as session:
            ...
            # Add current day
            current_equity = current_cash + sum(p["quantity"] * p["current_price"] for p in current_positions.values())
            equity_data.append((datetime.now().date(), current_equity))

            # Create series
            dates, values = zip(*equity_data, strict=False)
            return pd.Series(values, index=pd.DatetimeIndex(dates))

    def _calculate_rolling_sharpe(self, equity_curve: pd.Series, window_days: int = 30) -> float:
    ...
    def _calculate_rolling_sortino(self, equity_curve: pd.Series, window_days: int = 30) -> float:
    ...
    def _calculate_drawdown_metrics(self, equity_curve: pd.Series) -> tuple[float, float]:
    ...
    def _calculate_trade_statistics(self) -> Dict[str, Any]:
        ...
        with self.trade_db.get_session() as session:
            ...
            if not trades_pnl:
                return {"num_trades": 0, "num_winning": 0, "num_losing": 0, "win_rate": 0.0, "profit_factor": 0.0}
    ...
    def _build_historical_snapshot(self, snapshot_date: date) -> Optional[Dict[str, Any]]:
        ...
        with self.trade_db.get_session() as session:
            ...
            fills = (
                session.query(FillModel).filter(FillModel.timestamp <= end_datetime).order_by(FillModel.timestamp).all()
            )
            ...
            positions_value = sum(p["quantity"] * p["current_price"] for p in positions.values())
