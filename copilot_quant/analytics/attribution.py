...
class AttributionAnalyzer:
    ...
    def get_strategy_attribution(
        ...
        with self.trade_db.get_session() as session:
            ...
            query = session.query(FillModel).join(OrderModel, FillModel.order_db_id == OrderModel.id)

            if start_date:
                query = query.filter(FillModel.timestamp >= datetime.combine(start_date, datetime.min.time()))
            if end_date:
                query = query.filter(FillModel.timestamp <= datetime.combine(end_date, datetime.max.time()))
    ...
    def get_time_attribution(
        self, period: str = "daily", start_date: Optional[date] = None, end_date: Optional[date] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_strategy_trades(self, fills: List) -> Dict[str, Any]:
        ...
        return {"num_trades": len(trades_pnl), "win_rate": win_rate, "profit_factor": profit_factor}
